...
Raw YAML snippets demonstrating common `input_color` patterns. Copy what you
...
└── scenes/
    └── scene_capture.yaml         scene.create snapshot including input_color
```
...
- service: input_color.set_color
  target:
    entity_id: input_color.your_color
  data:
...
```yaml
{{ states('input_color.your_color') }}                       # "#FF8000"
{{ state_attr('input_color.your_color', 'rgb_color') }}      # [255, 128, 0]
{{ state_attr('input_color.your_color', 'hs_color') }}       # [30, 100]
{{ state_attr('input_color.your_color', 'kind') }}           # "chromatic" or "white"
{{ state_attr('input_color.your_color', 'color_temp_kelvin') }}  # int or null
```
