Following up on my earlier comment about the color-helper HACS integration: it's now been field-tested (thanks @Ltek and others), and I'd like to move this forward into core. Since my last comment I researched how core has handled helper integrations over the past few years, **updated the implementation to match core precedent** (domain renamed `input_color` → `color`, plus a new `color_params` attribute; details below), and **opened draft PRs**: [core#177605](https://github.com/home-assistant/core/pull/177605), [docs #47125](https://github.com/home-assistant/home-assistant.io/pull/47125), and [brands#10876](https://github.com/home-assistant/brands/pull/10876). Below is the architecture proposal behind them. I've been working on this problem intermittently for multiple years, first with an `input_text` helper holding a color string, parsed in scripts at apply time, then as the purpose-built integration. I'm offering to do the remaining work and ongoing code ownership.
...
1. **home-assistant/core** — **draft PR open: [core#177605](https://github.com/home-assistant/core/pull/177605)** (with [brands#10876](https://github.com/home-assistant/brands/pull/10876)). Fully async and typed, no third-party deps, config flow + options flow, `icons.json`/`strings.json` with translated selector labels, `ServiceValidationError` with translation keys and schema-level exactly-one-of color validation, 56 tests on the core test harness (100% coverage on `config_flow.py`, `__init__.py`, and `reproduce_state.py`), and hassfest, ruff, and mypy-strict all passing, rebased onto current `dev`. The field-tested [HACS implementation](https://github.com/kkilchrist/ha-color-ext) remains available for anyone who wants to try it today.
2. **home-assistant/frontend** — two findings. First, the Helpers picker needs **no frontend change at all**: the create-helper dialog builds its list dynamically from `getConfigFlowHandlers(hass, ["helper"])`, so any core integration with `integration_type: helper` and a config flow (threshold, derivative, and now `color`) appears automatically. Second, the more-info dialog is implemented, reusing the light dialog's building blocks (`ha-hs-color-picker` wheel, the same CCT gradient slider, brightness slider), per @Marconius6's suggestion above: branch at [kkilchrist/frontend@color-helper](https://github.com/kkilchrist/frontend/tree/color-helper), lint/type-check clean; I'll open that PR once the core PR is accepted, since it depends on the domain existing.
3. **home-assistant.io** — **draft PR open: [docs#47125](https://github.com/home-assistant/home-assistant.io/pull/47125)**, including the emit/apply truth tables (what `color_params` contains per state, and how the light integration maps it per `supported_color_modes`) plus example automations adapted from the HACS release.
...
The draft PRs ([core#177605](https://github.com/home-assistant/core/pull/177605), [docs#47125](https://github.com/home-assistant/home-assistant.io/pull/47125), [brands#10876](https://github.com/home-assistant/brands/pull/10876)) are up and ready for review, with the frontend more-info dialog prepared to follow. I'm happy to put in whatever remaining work is needed to get this over the finish line, plus ongoing code ownership, if I can get a commitment from one of the core maintainers that this is supported / supportable, and would be merged once feature complete.
