...
# back into color.set_color: e.g. {"rgb_color": [255, 158, 77]}. Read-only;
# every other color attribute not matching this shape is derived.
...
# Custom-integration extra, not part of the core proposal: a convenience
# dispatcher that pushes this color onto lights. Core users splat the
# `color_params` attribute into `light.turn_on` instead.
...
# apply_to fields. `lights` (not `target`) avoids visual collision with HA's
# top-level `target:` block in the YAML and visual editors.
